...
Compile under windows (using mingw):
    ...
    mingw32-cc.exe -o yuyv_tcp_server yuyv_tcp_server.c -lwsock32 -static
...
void handle_data(unsigned char *buf, int n) {
    if (n > 0) {
        if (pYUYV == NULL) {
            ...
            char filename[20];
            sprintf(filename, "%04d.raw", fileindex);
            fileindex++;
            pYUYV = fopen(filename,"wb");
            printf("Create file \"%s\"\n", filename);
        }
        fwrite(buf, sizeof(unsigned char), n, pYUYV);
    } else {
        if (pYUYV != NULL) {
